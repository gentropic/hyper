...
- **v0.4 (shipped)** — `gcu:img:<id>` container convention: dd-style runtime images render in their own Containers section, grouped by `runtime` field. Per-image Reset uses record-level IDB cleanup (`deleteIdbRecord`, `deleteIdbRecordsByPrefix`) so siblings sharing the runtime's DB are untouched. Establishes that the convention shape generalizes beyond `gcu:tool`.
